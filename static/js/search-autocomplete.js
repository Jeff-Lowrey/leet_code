  ...
  generateSuggestions(query, context) {
    ...
    switch (context.type) {
      case 'number_typing':
        ...
        if (suggestions.problems.length > 0) {
          suggestions.examples = [
            {
              text: `${context.number} difficulty=easy`,
              description: 'Find similar easy problems',
            },
      ...
      case 'filter_key_typing':
        // Suggest filter keys
        suggestions.filters = Object.values(this.availableFilters).filter((f) =>
          f.name.startsWith(context.partial.toLowerCase())
        );
        break;
