...
function testSolution() {
    ...
    console.assert(verifyAllPalindromes(result1),
        `Test 1 failed: not all parts are palindromes`);
    console.assert(verifyPartitionsComplete("aab", result1),
    ...
    console.assert(verifyAllPalindromes(result3),
    ...
    console.assert(verifyAllPalindromes(result5),
        `Test 5 failed: not all parts are palindromes`);

    // Test case 6: Mixed palindromes
    const result6 = solve("raceacar");
    console.assert(verifyAllPalindromes(result6),
        `Test 6 failed: not all parts are palindromes`);
    console.assert(verifyPartitionsComplete("raceacar", result6),
        `Test 6 failed: partitions don't reconstruct original string`);

    // Test case 7: All partitions are unique
    const result7 = solve("aab");
    const uniquePartitions = new Set(result7.map(partition => JSON.stringify(partition)));
    console.assert(uniquePartitions.size === result7.length,
        `Test 7 failed: found duplicate partitions`);

    // Test case 8: Longer palindrome
    const result8 = solve("abccba");
    const hasFullPalindrome = result8.some(partition =>
        partition.length === 1 && partition[0] === "abccba");
    console.assert(hasFullPalindrome,
        `Test 8 failed: should include full string as single palindrome`);
    console.assert(verifyAllPalindromes(result8),
